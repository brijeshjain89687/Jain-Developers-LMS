...
app.get('/api/health', (req, res) => res.json({ status: 'ok', db: 'firestore', project: 'jain-lms-f14cd', version: '3.0.0', render: !!process.env.RENDER, ts: new Date().toISOString() }));
...
app.use((err, req, res, next) => { console.error('❌', err.message); res.status(err.statusCode || 500).json({ error: err.message }); });

const PORT = process.env.PORT || 5000;
app.listen(PORT, '0.0.0.0', () => console.log(`🚀 Jain LMS :${PORT} | /student | /admin | /api/health`));
module.exports = app;
