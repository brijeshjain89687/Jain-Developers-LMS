...
// REQUIRED on Render: trust the proxy so express-rate-limit reads real IPs
...
// Rate limiting — standardHeaders prevents the legacy X-RateLimit-* warning
...
// ─── Static Files ────────────────────────────────────────────────────────────
// Render places server.js at /opt/render/project/src/server.js
// Try sibling public/ first (when backend/ IS the repo root),
// then fall back to ../public/ (when backend/ is a subfolder)
const publicSibling = path.join(__dirname, 'public');
const publicParent  = path.join(__dirname, '../public');
const resolvedPublic = fs.existsSync(publicSibling) ? publicSibling : publicParent;
  ...
  try {
    ...
    const courses = [
      ...
      },
      {
        ...
        projectDescription: 'Analyze a real-world sales dataset and create interactive visualizations.',
      ...
      },
      {
        ...
        sections: [
          { title: 'Docker & Containers', lessons: [
    ...
    const quizzes = [
      ...
      },
      {
        ...
        questions: [
          ...
          { question: 'Which keyword defines a function in Python?', options: ['function', 'def', 'func', 'define'], correctAnswer: 1, explanation: 'def is used to define functions.' },
        ],
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
      },
    ];
    ...
    const announcements = [
      ...
      { title: 'System Maintenance Tonight', body: 'The platform will be under brief maintenance from 2-3 AM IST. Please save your progress.', tag: 'Urgent', isActive: true, createdAt: admin.firestore.FieldValue.serverTimestamp() },
    ];
